...
export class PacketCaptureEngine extends EventEmitter {
  ...
  async start(): Promise<boolean> {
    try {
      ...
      // On Windows, iface may already be a \Device\NPF_{GUID} path — use it directly.
      // Otherwise try Cap.findDevice (looks up by IP) then fall back to the name itself.
      let device: string;
      if (this.iface.startsWith("\\Device\\") || this.iface.startsWith("/dev/")) {
        device = this.iface;
      } else {
        // Try matching by description in deviceList (handles "Wi-Fi", "Ethernet", etc.)
        const deviceList: Array<{ name: string; description: string; addresses: any[] }> =
          Cap.deviceList?.() ?? [];
        const match = deviceList.find(
          (d) =>
            d.description?.toLowerCase().includes(this.iface.toLowerCase()) ||
            d.name === this.iface
        );
        device = match?.name ?? Cap.findDevice(this.iface) ?? this.iface;
      }
