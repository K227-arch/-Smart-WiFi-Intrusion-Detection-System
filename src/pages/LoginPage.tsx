...
import { AlertCircle, ArrowLeft, Eye, EyeOff, Loader2, Mail, RefreshCw, ShieldCheck } from "lucide-react";
...
export function LoginPage({ onLogin }: LoginPageProps) {
    ...
    try {
      if (mode === "signup") {
        ...
        if (result.requireEmailVerification) {
          ...
          // No verification — sign in immediately
          const signInResult = await localAuth.signIn(email, password);
          if (signInResult.requireOtp) {
            setDevOtp(signInResult.devOtp || null);
            startCooldown();
            setScreen("otp");
          } else {
            onLogin();
  ...
  const switchMode = (m: Mode) => {
    setMode(m); setScreen("credentials");
    setError(null); setOtp(""); setDevOtp(null);
  };
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md relative z-10">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
          <AnimatePresence mode="wait">
            ...
            {/* ── Credentials ── */}
            {screen === "credentials" && (
              <motion.div key="creds" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="p-8">
                {/* Mode toggle */}
                ...
                <form onSubmit={handleCredentials} className="space-y-4">
                  {mode === "signup" && (
                    <div>
                      <label className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1.5 block">Full Name</label>
                  ...
                  <div>
                    <label className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1.5 block">Email</label>
                  ...
                  <div>
                    <label className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1.5 block">Password</label>
                    <div className="relative">
                  ...
                  <button type="submit" disabled={loading}
                    ...
                    {loading
                      ? wakingUp ? "Waking up server…" : "Please wait…"
            ...
            {screen === "otp" && (
              <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="p-8">
                <button onClick={() => { setScreen("credentials"); setError(null); setOtp(""); }}
                ...
                <div className="flex flex-col items-center mb-7">
                  ...
                  <p className="text-slate-400 text-sm mt-1.5 text-center">6-digit code sent to</p>
                ...
                <div className="mt-4 text-center">
                  <button onClick={handleResend} disabled={resendCooldown > 0}
                    className={cn("flex items-center gap-1.5 mx-auto text-xs font-semibold transition-colors",
                      resendCooldown > 0 ? "text-slate-600 cursor-not-allowed" : "text-amber-400 hover:text-amber-300")}>
                    <RefreshCw className="w-3 h-3" />
                    {resendCooldown > 0 ? `Resend in ${resendCooldown}s` : "Resend code"}
                  </button>
                </div>

                {/* Dev mode: show OTP inline when no email service */}
                {devOtp && (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                    className="mt-5 flex flex-col items-center gap-2 p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl">
                    <p className="text-[10px] text-amber-400 font-bold uppercase tracking-widest">Your verification code</p>
                    <div className="text-4xl font-mono font-black tracking-[0.3em] text-amber-300 select-all">{devOtp}</div>
                    <p className="text-[9px] text-amber-600 text-center">No email service configured — code shown here for convenience.</p>
                    <button type="button" onClick={() => { setOtp(devOtp); setError(null); }}
        ...
        <p className="text-center text-slate-600 text-xs mt-6 font-mono">
          SALAMANDA NIDS v2.0 — Secured by local auth
        </p>
      </motion.div>
    </div>
  );
