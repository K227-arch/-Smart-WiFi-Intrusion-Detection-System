...
import { LoginPage, checkLocalAuth, localSignOut } from "./pages/LoginPage";
...
function Dashboard({ onSignOut }: { onSignOut: () => void }) {
  ...
  useEffect(() => {
    const token = localStorage.getItem("wids_local_token");
    if (!token) return;
    fetch("/api/local-auth/me", { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => r.json())
      .then(({ user }) => {
